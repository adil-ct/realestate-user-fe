...
import MKButtonRoot from "components/custom/MKButton/MKButtonRoot";

const MKButton = forwardRef(
  (
    { color, variant, size, circular, iconOnly, children, customSize, ...rest },
    ref
  ) => (
    <MKButtonRoot
      {...rest}
      ...
      size={size}
      ownerState={{ color, variant, size, circular, iconOnly, customSize }}
    >
      {children}
    </MKButtonRoot>
  )

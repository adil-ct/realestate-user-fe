      ...
      {row?.map((data) => (
        <React.Fragment key={nanoid()}>
          <Grid container columns={16} className={classes.cardContainer}>
            <React.Fragment key={nanoid()}>
              <Grid item xs={8}>
                <Typography
                  variant="h6"
                  fontSize="14px"
                  className={classes.keyHeadingMain}
                >
                  {data["Date & Time"]}
                </Typography>
              </Grid>
              <Grid item xs={8} className={classes.wordBreak1}>
                <Typography
                  variant="h6"
                  fontSize="14px"
            ...
            {column
              .filter((item) => item !== "Date & Time" && item !== "Status")
              .map((key, index) => (
                <React.Fragment key={nanoid()}>
                  <Grid item xs={8}>
                    <Typography

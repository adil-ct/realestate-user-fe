...
const ReferralDetails = () => {
  ...
  const { userData, isLoading: profileLoader } = useSelector(
    (state) => state.auth,
  );
  const { isLoading, refreeDetails } = useSelector((state) => state.user);

  const affiliateLink =
    REFERRAL_BASE_URL + refreeDetails?.data?.referralCodeDetails?.referralCode;
  ...
  const { copy: referralIDCopy, tooltipText: referralIDText } =
    useCopyToClipboard();
  const { copy: referralLinkCopy, tooltipText: referralLinkText } =
    useCopyToClipboard();

  const maxCount = refreeDetails?.data?.totalCount
    ? Math.ceil(Number(refreeDetails?.data?.totalCount / 10))
    : 0;
  ...
  useEffect(() => {
    dispatch(
      getRefreeDetails({
        endpoint: `/user/refereeList/?page=${currentPage}&limit=10`,
      }),
    );
  }, [currentPage]);
    ...
    <Box className={classes.paddingContainer}>
      {!profileLoader?.profile ? (
        <Box className={classes.mainContainer}>
          {!affiliateUser && (
            <Grid container mt={2} className={classes.referralDetailBox}>
              <Grid item md={12} lg={6} xl={6}>
                <Box className={classes.referralItem1}>
                  <Box className={classes.referralLinkBox}>
                    ...
                    <Box className={classes.referralShare}>
                      Invite your friends to sign up and invest
                    </Box>
                  </Box>
                  <Grid
                    container
                    spacing={2}
                    className={classes.refMainContainer}
                  >
                    <Grid
                      item
                      sm={12}
                        ...
                        <Box style={{ width: "100%" }}>
                          ...
                          >
                            <MKTypography className={classes.referralIdValue}>
                              {refreeDetails?.data?.referralCodeDetails
                                ?.referralCode ?? "-"}
                            </MKTypography>
                    ...
                    >
                      <Box className={classes.referralIdBox}>
                        <Box style={{ width: "100%" }}>
                          ...
                          >
                            <MKTypography className={classes.referralIdValue}>
                              {affiliateLink?.length > 10
                                ? affiliateLink?.slice(0, 8) +
                                  "....." +
                                  affiliateLink?.slice(-4)
                                : affiliateLink}
                            </MKTypography>
                            <Box className={classes.iconCotainer}>
                              <Tooltip
              ...
              <Grid item md={12} lg={6} xl={6}>
                <Box className={classes.referralItem3}>
                  <Box className={classes.referralLinkBox}>
                    ...
                    <Box className={classes.referralShare}>
                      Get $5 each when your friend makes their first investment.
                    </Box>
                  </Box>
                </Box>
              </Grid>
            ...
            <Grid item xs={6} md={6} lg={3}>
              <Box className={classes.marketStats}>
                <Box>
                  ...
                  <MKTypography className={classes.marketStatsVal}>
                    {refreeDetails?.data?.referralCodeDetails?.totalReferee ??
                      0}
                  </MKTypography>
                </Box>
              </Box>
          ...
          <Grid container spacing={2} className={classes.gridTableContainer}>
            <Grid item xs={12}>
              <MKBox mt={2}>
                {!isLoading.refreeDetails &&
                refreeDetails?.data?.result?.length === 0 ? (
                  ...
                  <Box className={classes.sectionDesktopFlex}>
                    <div className={classes.tableContainer}>
                      {isLoading?.refreeDetails ? (
                        <TableSkeleton dynamicField={true} {...affiliateData} />
                      ) : (
                        <DynamicTable
                          height="100%"
                          {...affiliateData}

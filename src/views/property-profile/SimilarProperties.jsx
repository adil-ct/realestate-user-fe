...
import SimilarPropStyles from "./similerPropStyles";
...
const SimilarProperties = ({id}) => {
  const classes = SimilarPropStyles();
  const isLogedin = localStorage.getItem("authToken");
  const { similarPropertyList, isLoading } = useSelector((state) => state.marketplace);
  ...
  useEffect(() => {
    ...
    dispatch(
      getSimilarPropertyListSaga({
        id,
        qp: "startIndex=1&itemsPerPage=100",
  ...
  useEffect(() => {
    ...
    if (similarPropertyList && similarPropertyList?.length) {
      for (let i = 0; i < similarPropertyList.length; i++) {
        if (similarPropertyList[i]?.trending) {
          trendingPropertiesTemp.push(similarPropertyList[i]);
        } else {
          upcomingPropertiesTemp.push(similarPropertyList[i]);
        }
      }
